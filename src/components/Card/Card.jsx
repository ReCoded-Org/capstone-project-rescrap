...
const Card = ({
  ...
  return (
    <div className="flex w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 flex-col bg-white border rounded-lg overflow-hidden shadow-lg mx-4 my-4">
      <a href={productDetailsLink} target="_blank" alt={cardTitle}>
        <img
      ...
      <div className="p-4">
        ...
        <div className="flex flex-row justify-between">
          <span className="ml-1 inline-block bg-green-500 text-white text-sm px-2 rounded-full">
            {cardLocation}
          </span>{' '}
          <h4 className="text-green-600">{cardPrice}</h4>
        </div>
      </div>
    </div>
  );
